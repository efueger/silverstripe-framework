...
This can be customised in one of the following ways by editing your user code:
 ...
 * `canViewDraft` to update the visibility of this object only in draft mode.
 * `canViewArchive` to update the visibility of this object only in archive mode.
 
Note that unlike canViewVersioned, the canViewDraft or canViewArchived will 
only be invoked if the object is confirmed to be i the appropriate stage, so
the stage check is unnecessary.
...
E.g.
    ...
    class MyObjectExtension extends DataExtension {
        ...
        public function canViewArchive($member = null) {
            return Permission::check($member, 'ARCHIVE_STATUS');
        }
    }
