...
```php
	class GalleryPage extends Page {
		...
		function getCMSFields() {
			...
			// Restrict validator to include only images
...
This can be done by using file category names. These include  'audio', 'doc', 'flash', 'image', 'unresizeable_image',
'mov', and 'zip'.

Note that 'image' will limit to only images which the SilverStripe image manipulations API is able to 
process, which are jpeg, jpg, png, or gif. Other image formats (some of which may not be compatible
with the visitor's browser) can also be included via 'unresizeable_image' category.


	:::php
	$uploadField->setAllowedFileCategories('image', 'unresizeable_image', 'doc');


This will limit files to the following extensions: bmp gif jpg jpeg pcx tif png alpha als cel
icon ico ps doc docx txt rtf xls xlsx pages ppt pptx pps csv html htm xhtml xml pdf.
 ...
 * `setAllowedFileCategories`: (array|string) List of types of files allowed. May be any of
   'image', 'audio', 'mov', 'zip', 'flash', or 'doc'.
...
*In GalleryPage.php:*
	...
	class GalleryPage_Controller extends Page_Controller {
		...
		public function Form() {
			...
			$field->setAllowedFileCategories('image'); // Allow images only
