...
appropriate mime types. The following regeneration and cleaning classes and methods have been removed:
...
an `Image` datatype, or refer to `DBFile('image')`.
...
in migration of legacy files. Migration will automatically occur during dev/build. In some cases, it may be necessary
to disable this automatic migration. This can be disabled via the below config setting:
