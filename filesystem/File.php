...
class File extends DataObject implements ShortcodeHandler, AssetContainer {
	...
	private static $app_categories = array(
		...
		'mov' => array(
			"mpeg" ,"mpg" ,"mp4" ,"m1v" ,"mp2" ,"mpa" ,"mpe" ,"ifo" ,"vob","avi" ,"wmv" ,"asf" ,"m2v" ,"qt", "ogv",
			"webm", "mov"
		),
		'zip' => array(
			"arc" ,"rar" ,"tar" ,"gz" ,"tgz" ,"bz2" ,"dmg" ,"jar","ace" ,"arj" ,"bz" ,"cab", "zip"
		),
		'image' => array(
	...
	/**
	 * Mapping of file categories to allowed upload types. Falls back to app_categories for any undefined categories.
	 *
	 * @config
	 * @var array
	 */
	private static $upload_categories = array(
		// Only image formats compatible with {@see Image_Backend}
		// {@see DBFile::$supported_images} for mime types
		'image' => array(
			'gif', 'jpg', 'jpeg', 'png'
		),
		'unresizeable_image' => array(
			'bmp', 'pcx', 'tif', 'alpha', 'als', 'cel', 'icon', 'ico', 'ps'
		)
	);
	 ...
	 * @var bool
	 */
	private static $migrate_legacy_file = true;
	...
	/**
	 * For a given category or list of categories, map to file extensions that upload can process.
	 *
	 * Respects upload_categories and uses app_categories as a fallback.
	 *
	 * @param array|string $categories
	 * @return array
	 */
	public static function get_upload_extensions($categories) {
		if(empty($categories)) {
			return array();
		}
		...
		$uploadCategories = self::config()->upload_categories;
		...
		foreach(array_filter($categories) as $category) {
			if(isset($uploadCategories[$category])) {
				$extensions = array_merge($extensions, $uploadCategories[$category]);
			} elseif(isset($appCategories[$category])) {
