...
$.entwine('ss', function($) {
	...
	$('div.ss-upload').entwine({
		...
		onmatch: function() {
			...
			var ssDropzone = new Dropzone($dropZone[0], { 
				url: config['url'],
		...
		openSelectDialog: function(uploadedFile) {
			...
			var self = this, config = this.find('.ss-uploadfield-fromcomputer-fileinput').data('config'), dialogId = 'ss-uploadfield-dialog-' + this.attr('id'), dialog = jQuery('#' + dialogId);
		...
		attach: function(data) {
			// Handles attachment of already uploaded files
			var self = this,
				files = data.files,
