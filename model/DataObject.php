...
class DataObject extends ViewableData implements DataObjectInterface, i18nEntityProvider {
	 ...
	 * @param array &$compositeFields List of composite fields which do not have an actual DB column, but rather
	 * are split into multiple columns. Those child columns will be included in the returned array, however.
	 * @return array Map of fieldname to specification, similiar to {@link DataObject::$db}.
	 */
	public static function database_fields($class = null, &$compositeFields = array()) {
		if(empty($class)) {
	 ...
	 * @return array Map of fieldname to specification, similiar to {@link DataObject::$db}.
	 */
	public static function custom_database_fields($class = null, &$compositeFields = array()) {
		if(empty($class)) {
			$class = get_called_class();
		}
		
		// Get all fields
		$fields = self::database_fields($class, $compositeFields);
	...
	public static function composite_fields($class = null, $aggregated = true) {
		...
		self::database_fields($class, $compositeFields);
		if(!$aggregated) {
	...
	public function dbObject($fieldName) {
		...
		$helper = $this->db($fieldName, $table);
		if($helper) {
