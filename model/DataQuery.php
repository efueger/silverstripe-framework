...
class DataQuery {
	...
	public function getFinalisedQuery($queriedColumns = null) {
		...
		$baseIDColumn = $schema->quotedID($baseDataClass);
		...
		foreach($tableClasses as $tableClass) {
			...
			if($tableClass !== $baseDataClass) {
				...
				$tableIDColumn = $schema->quotedID($tableClass);
				$query->addLeftJoin(
					$tableName,
					"{$tableIDColumn} = {$baseIDColumn}",
					$tableName,
					10
				);
			}
			...
			foreach($this->collidingFields as $collisionField => $collisions) {
				...
				foreach($collisions as $collision) {
					if(preg_match('/^"(?<table>[^"]+)"\./', $collision, $matches)) {
						...
						if($collisionClass) {
							$collisionClassColumn = $schema->quotedColumn($collisionClass, 'ClassName');
		...
		if($this->filterByClassName) {
			...
			if($this->dataClass != $baseDataClass) {
				...
				$baseClassColumn = $schema->quotedColumn($baseDataClass, 'ClassName');
		...
		if($orderby = $query->getOrderBy()) {
			...
			foreach($orderby as $k => $dir) {
				...
				if(count($parts) == 1) {
					...
					$qualCol = DataObject::getSchema()->quotedColumn($this->dataClass(), $parts[0]);;
					if(!$qualCol) {
						$qualCol = "\"$parts[0]\"";
					}
	...
	public function count() {
		$quotedColumn = DataObject::getSchema()->quotedColumn($this->dataClass(), 'ID');
	...
	protected function selectColumnsFromTable(SQLSelect &$query, $tableClass, $columns = null) {
		...
		foreach($databaseFields as $k => $v) {
			if((is_null($columns) || in_array($k, $columns)) && !isset($compositeFields[$k])) {
				...
				$quotedField = DataObject::getSchema()->quotedColumn($tableClass, $k);
	...
	protected function joinHasOneRelation($localClass, $localField, $foreignClass)
	{
		...
		// Join base table
		$foreignIDColumn = $schema->quotedID($foreignBaseClass);
		$localColumn = $schema->quotedColumn($localClass, "{$localField}ID");
		...
		if(!empty($ancestry)){
			...
			foreach($ancestry as $ancestor){
				...
				if($ancestorTable !== $foreignBaseTable) {
					$ancestorIDColumn = $schema->quotedID($ancestor);
					$this->query->addLeftJoin($ancestorTable, "{$foreignIDColumn} = {$ancestorIDColumn}");
				}
			}
		}
	}
	...
	protected function joinHasManyRelation($localClass, $localField, $foreignClass) {
		...
		$localIDColumn = $schema->quotedColumn($localClass, 'ID');
		if($polymorphic) {
			$foreignKeyIDColumn = $schema->quotedColumn($foreignClass, "{$foreignKey}ID");
			$foreignKeyClassColumn = $schema->quotedColumn($foreignClass, "{$foreignKey}Class");
			$localClassColumn = $schema->quotedColumn($localClass, 'ClassName');
			$this->query->addLeftJoin(
				$foreignTable,
				"{$foreignKeyIDColumn} = {$localIDColumn} AND {$foreignKeyClassColumn} = {$localClassColumn}"
			);
		} else {
			$foreignKeyIDColumn = $schema->quotedColumn($foreignClass, $foreignKey);
		...
		$ancestry = ClassInfo::ancestry($foreignClass, true);
		$ancestry = array_reverse($ancestry);
		$foreignIDColumn = $schema->quotedID($foreignClass);
		foreach($ancestry as $ancestor) {
			$ancestorTable = $schema->tableName($ancestor);
			if($ancestorTable !== $foreignTable) {
				$ancestorIDColumn = $schema->quotedID($ancestor);
				$this->query->addInnerJoin($ancestorTable, "{$foreignIDColumn} = {$ancestorIDColumn}");
			}
		}
	}
	...
	protected function joinManyManyRelationship($parentClass, $componentClass, $parentField, $componentField, $relationTable) {
		...
		$parentIDColumn = $schema->quotedColumn($parentClass, 'ID');
		$this->query->addLeftJoin(
			$relationTable,
		...
		$componentIDColumn = $schema->quotedID($componentBaseClass);
		...
		foreach($ancestry as $ancestor) {
			...
			if($ancestorTable != $componentBaseTable && !$this->query->isJoinedTo($ancestorTable)) {
				$ancestorIDColumn = $schema->quotedID($ancestor);
				$this->query->addLeftJoin($ancestorTable, "{$componentIDColumn} = {$ancestorIDColumn}");
			}
		}
	}
	...
	protected function expressionForField($field) {
		...
		if($field === 'ID') {
			return DataObject::getSchema()->quotedColumn($this->dataClass, 'ID');
		}
		return null;
	}
