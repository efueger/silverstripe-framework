...
class Versioned extends DataExtension implements TemplateGlobalProvider {
	...
	public function canViewVersioned($member = null) {
		...
		$extended = null;
		if($mode === 'stage') {
			$extended = $this->owner->extendedCan('canViewDraft', $member);
		} elseif($mode === 'archive') {
			$extended = $this->owner->extendedCan('canViewArchive', $member);
		}
		if($extended !== null) {
	 ...
	 *
	 * Special considerations for this method:
	 * - This method should not be called via ->extend('canViewStage'), but rather
	 *   should be, either overridden in the extended class, or extended with
	 *   updateCanViewStage().
	 * - This method should normally be called by canView if in a stage other than
	 *   Live, and should not be used in isolation to determine if a user
	 *   has specific viewing permission for a record in a stage.
	 *
	 * @param string $stage
	 * @param Member $member
	 * @return bool
